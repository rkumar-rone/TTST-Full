 ...
 */
export default class saltboxCartItems extends NavigationMixin(LightningElement) {
    /**
    ...
    set cartItems(items) {
        ...
        this._items = (items || []).map((item) => {
            ...
            newItem.cartItem.productDetails.name = newItem.cartItem.productDetails.name.replace(/&amp;/g,'&');
    ...
    get displayItems() {
        return this._items.map((item) => {
            ...
            let hasDiscount = Number(item.cartItem.totalAdjustmentAmount) !== 0;
            let tempTotalAmount = Number(item.cartItem.quantity) * Number(item.cartItem.salesPrice);

            //! this will be the correct one
            let calculatedAmount = tempTotalAmount + Number(item.cartItem.totalAdjustmentAmount);
            //! temp fix
            //let calculatedAmount = tempTotalAmount;

            const newItem = { ...item };
            newItem.cartItem.hasDiscount = hasDiscount;
            newItem.cartItem.calculatedAmount = tempTotalAmount;
            newItem.cartItem.totalPrice = calculatedAmount;;
