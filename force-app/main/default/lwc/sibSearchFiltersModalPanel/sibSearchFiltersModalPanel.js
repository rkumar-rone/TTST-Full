...
export default class SearchFiltersModalPanel extends LightningElement {
    ...
    _transformCategoryAndFacetData(res) {
        ...
        if (categories && facets) {
            const processedCategories = this.createCategoryTree(categories);
            const processedFacets = this.transformInputFacets(facets);
            this.searchResults = {
                searchCategory: processedCategories,
                filters: processedFacets,
            };
        }
    }

    /**
     * Translate category data into UI representations
     * @param {searchCategoryData} searchCategoryData - Categories Response returned from the ProductSearchAdapter
     */
    createCategoryTree(searchCategoryData) {
        // Map categories
        const searchCategoryConnect = searchCategoryData.category;
        let searchCategory = {
            id: searchCategoryConnect.id,
            name: searchCategoryConnect.name,
            productCount: searchCategoryData.productCount,
            description: searchCategoryConnect.description,
            urlSlug: searchCategoryConnect.urlName,
            children: []
        };

        // Map child categories
        if (searchCategoryData.children && searchCategoryData.children.length > 0) {
            searchCategory.children = searchCategoryData.children.map(child => ({
                id: child.category.id,
                name: child.category.name,
                productCount: child.category.productCount,
                description: child.category.description,
                urlSlug: child.category.urlName
            }));
        }
    ...
    /**
     * Translate facet data into UI representations
     * @param {searchFilters} searchFilters - facet Response returned from the ProductSearchAdapter
     */
    transformInputFacets(searchFilters) {
        // Map facets
        const filters = searchFilters.map(facet => {
            let filter = {
                attributeType: facet.attributeType,
                displayName: facet.displayName,
                displayRank: facet.displayRank,
            ...
            // Sort only if nameOrId is "Month_Year__c"
