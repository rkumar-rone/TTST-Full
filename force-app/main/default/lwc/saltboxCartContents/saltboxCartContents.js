...
import getCartItems from '@salesforce/apex/Saltbox_B2BCartController2.getCartItems';
import updateCartItem from '@salesforce/apex/Saltbox_B2BCartController2.updateCartItem';
import deleteCartItem from '@salesforce/apex/Saltbox_B2BCartController2.deleteCartItem';
import deleteCart from '@salesforce/apex/Saltbox_B2BCartController2.deleteCart';
import createCart from '@salesforce/apex/Saltbox_B2BCartController2.createCart';
...
// Locked Cart Status
const LOCKED_CART_STATUSES = new Set(['Checkout']); //new Set(['Processing', 'Checkout']);
...
export default class saltboxCartContents extends NavigationMixin(LightningElement) {
    /**
    ...
    pushGtmViewCartEvent() {
        ...
        try {
            //View Cart GTM 
            getCartPromotions({ cartId: this.recordId })
            .then((result) => {
                ...
                //console.log('this.cartItems', JSON.parse(JSON.stringify(items)));
            ...
            console.log('Error in GTM View Cart', e);
        }
    ...
    updateCartItems() {
        try {
            ...
            .then((result) => {
                //console.log('Cart Summary', JSON.parse(JSON.stringify(result)));
                let cartSummary = JSON.parse(JSON.stringify(result));
                this.cartSummary = cartSummary;
                if (cartSummary.status == 'Checkout') {
                    isCartOnCheckout = true;
                }
            })
            .then(() => {
                if (isCartOnCheckout == false) {
                    // Call the 'getCartItems' apex method imperatively
                    calculatePromotions({cartId: this.recordId}).then(() => {
                        getCartItems({
                            communityId: communityId,
                            effectiveAccountId: this.resolvedEffectiveAccountId,
                            activeCartOrId: this.recordId,
            ...
            if (error.body.stackTrace.contains('calculatePromotions')) {
                console.error('Bad checkout close');
            } else {
                console.error('Error in cart contents component', error);
            }
        }
    ...
    removeCartItem(cartItemId) {
        ...
        /*console.log('removedItem', JSON.parse(JSON.stringify(removedItem)));
        console.log('quantityOfRemovedItem', quantityOfRemovedItem);*/
        ...
        try {
            ...
            .then((result) => {
                ...
                cartPromotions.cartItemPromotions.forEach(itemPromotion => {
                    if (itemPromotion.cartItemId == cartItemId) {
                ...
                let auxItem = { item_name:removedItem.cartItem.productDetails.name, item_id:removedItem.cartItem.productDetails.sku};

                items.push(auxItem);

                let value = removedItem.cartItem.totalAmount;
                let currency = removedItem.cartItem.currencyIsoCode;
            ...
            console.log('Error in GTM Remove from Cart', e);
        }
