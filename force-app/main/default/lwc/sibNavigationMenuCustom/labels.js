import SIB_SelfStudy from '@salesforce/label/c.SIB_SelfStudy';
import SIB_PublicCourses from '@salesforce/label/c.SIB_PublicCourses';
import SIB_CorporateTraining from '@salesforce/label/c.SIB_CorporateTraining';
import SIB_Academic from '@salesforce/label/c.SIB_Academic';
import SIB_AboutUs from '@salesforce/label/c.SIB_AboutUs';
import SIB_Blog from '@salesforce/label/c.SIB_Blog';
import SIB_FreeResources from '@salesforce/label/c.SIB_FreeResources';
import SIB_Fundamentals from '@salesforce/label/c.SIB_Fundamentals';
import SIB_Fundamentals_Info from '@salesforce/label/c.SIB_Fundamentals_Info';
import SIB_Foundations from '@salesforce/label/c.SIB_Foundations';
import SIB_Foundations_Info from '@salesforce/label/c.SIB_Foundations_Info';
import SIB_Advanced from '@salesforce/label/c.SIB_Advanced';
import SIB_Advanced_Info from '@salesforce/label/c.SIB_Advanced_Info';
import SIB_FoundationsPlusAdvanced from '@salesforce/label/c.SIB_FoundationsPlusAdvanced';
import SIB_FoundationsPlusAdvanced_Info from '@salesforce/label/c.SIB_FoundationsPlusAdvanced_Info';
import SIB_InterviewPrep from '@salesforce/label/c.SIB_InterviewPrep';
import SIB_InterviewPrep_Info from '@salesforce/label/c.SIB_InterviewPrep_Info';
import SIB_InterviewFundamentals from '@salesforce/label/c.SIB_InterviewFundamentals';
import SIB_IBInterviewPrep from '@salesforce/label/c.SIB_IBInterviewPrep';
import SIB_PEInterviePrep from '@salesforce/label/c.SIB_PEInterviePrep';
import SIB_HedgeFundInterviewPrep from '@salesforce/label/c.SIB_HedgeFundInterviewPrep';
import SIB_RolePrep from '@salesforce/label/c.SIB_RolePrep';
import SIB_RolePrep_Info from '@salesforce/label/c.SIB_RolePrep_Info';
import SIB_InvestmentBanking from '@salesforce/label/c.SIB_InvestmentBanking';
import SIB_Private_Equity from '@salesforce/label/c.SIB_Private_Equity';
import SIB_EquityResearch from '@salesforce/label/c.SIB_EquityResearch';
import SIB_BondMarket from '@salesforce/label/c.SIB_BondMarket';
import SIB_Assetmanagement from '@salesforce/label/c.SIB_Assetmanagement';
import SIB_GlobalMarketandHedgeFund from '@salesforce/label/c.SIB_GlobalMarketandHedgeFund';
import SIB_Restructuring from '@salesforce/label/c.SIB_Restructuring';
import SIB_IndustrySpecific from '@salesforce/label/c.SIB_IndustrySpecific';
import SIB_RealEstate from '@salesforce/label/c.SIB_RealEstate';
import SIB_SoftwareTech from '@salesforce/label/c.SIB_SoftwareTech';
import SIB_OilGas from '@salesforce/label/c.SIB_OilGas';
import SIB_BankingFIG from '@salesforce/label/c.SIB_BankingFIG';
import SIB_Insurance from '@salesforce/label/c.SIB_Insurance';
import SIB_PharmaBiotech from '@salesforce/label/c.SIB_PharmaBiotech';
import SIB_Productivity from '@salesforce/label/c.SIB_Productivity';
import SIB_Productivity_Info from '@salesforce/label/c.SIB_Productivity_Info';
import SIB_Mastering_Excel from '@salesforce/label/c.SIB_Mastering_Excel';
import SIB_BecomeProficientInPowerPoint from '@salesforce/label/c.SIB_BecomeProficientInPowerPoint';
import SIB_GettingStartedwithCopilot from '@salesforce/label/c.SIB_GettingStartedwithCopilot';
import SIB_OneOnOneMentoring from '@salesforce/label/c.SIB_OneOnOneMentoring';
import SIB_SelfStudyFreeResources from '@salesforce/label/c.SIB_SelfStudyFreeResources';
import SIB_ViewAllSelfStudy_Btn from '@salesforce/label/c.SIB_ViewAllSelfStudy_Btn';
import SIB_PublicCourses_MenuItem from '@salesforce/label/c.SIB_PublicCourses_MenuItem';
import SIB_PublicCourses_MenuItem_Info from '@salesforce/label/c.SIB_PublicCourses_MenuItem_Info';
import SIB_EssentialComprehensive from '@salesforce/label/c.SIB_EssentialComprehensive';
import SIB_AppliedExcel from '@salesforce/label/c.SIB_AppliedExcel';
import SIB_FinancialStatementAnalysisAndAccounting from '@salesforce/label/c.SIB_FinancialStatementAnalysisAndAccounting';
import SIB_PrivateEquityTransition from '@salesforce/label/c.SIB_PrivateEquityTransition';
import SIB_FinancialModeling from '@salesforce/label/c.SIB_FinancialModeling';
import SIB_EnhancedEssential from '@salesforce/label/c.SIB_EnhancedEssential';
import SIB_WallStreetBootcamp from '@salesforce/label/c.SIB_WallStreetBootcamp';
import SIB_PrinciplesOfCapitalMarkets from '@salesforce/label/c.SIB_PrinciplesOfCapitalMarkets';
import SIB_EveningEssentials from '@salesforce/label/c.SIB_EveningEssentials';
import SIB_PythonTraining from '@salesforce/label/c.SIB_PythonTraining';
import SIB_CopilotFoundations from '@salesforce/label/c.SIB_CopilotFoundations';
import SIB_AdvancedTopics from '@salesforce/label/c.SIB_AdvancedTopics';
import SIB_SoftwareIndustryAnalysisFinancialModeling from '@salesforce/label/c.SIB_SoftwareIndustryAnalysisFinancialModeling';
import SIB_RealEstateFundamentals from '@salesforce/label/c.SIB_RealEstateFundamentals';
import SIB_ViewAllPublicCourses_Btn from '@salesforce/label/c.SIB_ViewAllPublicCourses_Btn';
import SIB_CorporateSolutions from '@salesforce/label/c.SIB_CorporateSolutions';
import SIB_CorporateSolutions_Info from '@salesforce/label/c.SIB_CorporateSolutions_Info';
import SIB_CorporateOverview from '@salesforce/label/c.SIB_CorporateOverview';
import SIB_CorporateExcelTraining from '@salesforce/label/c.SIB_CorporateExcelTraining';
import SIB_ContinuingDevelopment from '@salesforce/label/c.SIB_ContinuingDevelopment';
import SIB_ProgramAdvisoryServices from '@salesforce/label/c.SIB_ProgramAdvisoryServices';
import SIB_PrivateEquity from '@salesforce/label/c.SIB_PrivateEquity';
import SIB_GlobalMarkets from '@salesforce/label/c.SIB_GlobalMarkets';
import SIB_FinancialStatementAnalysis from '@salesforce/label/c.SIB_FinancialStatementAnalysis';
import SIB_CopilotForFinance from '@salesforce/label/c.SIB_CopilotForFinance';
import SIB_DataScienceAndAnalytics from '@salesforce/label/c.SIB_DataScienceAndAnalytics';
import SIB_ModelingSolutions from '@salesforce/label/c.SIB_ModelingSolutions';
import SIB_ProfessionalSkillsTraining from '@salesforce/label/c.SIB_ProfessionalSkillsTraining';
import SIB_ViewAllCorporateSolutions_Btn from '@salesforce/label/c.SIB_ViewAllCorporateSolutions_Btn';
import SIB_OtherResource from '@salesforce/label/c.SIB_OtherResource';
import SIB_IndustrySpecific_Info from '@salesforce/label/c.SIB_IndustrySpecific_Info';
import SIB_SignIn from '@salesforce/label/c.SIB_SignIn';
import SIB_HeaderPromoBanner from '@salesforce/label/c.SIB_HeaderPromoBanner';
import SIB_LearnMore from '@salesforce/label/c.SIB_LearnMore';
import SIB_Back from '@salesforce/label/c.SIB_Back';
import SIB_UserMenu_Profile from '@salesforce/label/c.SIB_UserMenu_Profile';
import SIB_UserMenu_Logout from '@salesforce/label/c.SIB_UserMenu_Logout';
import SIB_UserMenu_Orders from '@salesforce/label/c.SIB_UserMenu_Orders';
import SIB_UserMenu_MyCourses from '@salesforce/label/c.SIB_UserMenu_MyCourses';
import SIB_UserMenu_ContactSupport from '@salesforce/label/c.SIB_UserMenu_ContactSupport';
import SIB_CourseCalendar from '@salesforce/label/c.SIB_CourseCalendar';
import SIB_PublicCourseCalendar from '@salesforce/label/c.SIB_PublicCourseCalendar';
import SIB_PublicCourseCalendar_Info from '@salesforce/label/c.SIB_PublicCourseCalendar_Info';
import MegaMenuId from '@salesforce/label/c.SIB_MegaMenuIdName';
import SIB_HomePageUrl from '@salesforce/label/c.SIB_HomePageUrl';
import SIB_MostPopularName from '@salesforce/label/c.SIB_MostPopularName';
import SIB_SearchPlaceHolderText from '@salesforce/label/c.SIB_SearchPlaceHolderText';
import SIB_LWRSiteURL from '@salesforce/label/c.SIB_LWRSiteURL';
import SIB_SiteDomainName from '@salesforce/label/c.Cje_Domain_Name';
import SIB_StrivacityIdCookieName from '@salesforce/label/c.SIB_StrivacityIdCookieName';
import SIB_ExcelModellingChallenges from '@salesforce/label/c.SIB_ExcelModellingChallenges';
import SIB_PythonCoreDataAnalysis from '@salesforce/label/c.SIB_PythonCoreDataAnalysis';

export const Labels ={
    SIB_SearchPlaceHolderText,
    SIB_PublicCourseCalendar_Info,
    SIB_MostPopularName,
    SIB_HomePageUrl,
    MegaMenuId,
    SIB_CourseCalendar,
    SIB_PublicCourseCalendar,
    SIB_UserMenu_Profile,
    SIB_UserMenu_Logout,
    SIB_UserMenu_Orders,
    SIB_UserMenu_MyCourses,
    SIB_UserMenu_ContactSupport,
    SIB_Back,
    SIB_LearnMore,
    SIB_HeaderPromoBanner,
    SIB_SignIn,
    SIB_IndustrySpecific_Info,
    SIB_OtherResource,
    SIB_SelfStudy,
    SIB_PublicCourses,
    SIB_CorporateTraining,
    SIB_Academic,
    SIB_AboutUs,
    SIB_Blog,
    SIB_FreeResources,
    SIB_Fundamentals,
    SIB_Fundamentals_Info,
    SIB_Foundations,
    SIB_Foundations_Info,
    SIB_Advanced,
    SIB_Advanced_Info,
    SIB_FoundationsPlusAdvanced,
    SIB_FoundationsPlusAdvanced_Info,
    SIB_InterviewPrep,
    SIB_InterviewPrep_Info,
    SIB_InterviewFundamentals,
    SIB_IBInterviewPrep,
    SIB_PEInterviePrep,
    SIB_HedgeFundInterviewPrep,
    SIB_RolePrep,
    SIB_RolePrep_Info,
    SIB_InvestmentBanking,
    SIB_Private_Equity,
    SIB_EquityResearch,
    SIB_BondMarket,
    SIB_Assetmanagement,
    SIB_GlobalMarketandHedgeFund,
    SIB_Restructuring,
    SIB_IndustrySpecific,
    SIB_RealEstate,
    SIB_SoftwareTech,
    SIB_OilGas,
    SIB_BankingFIG,
    SIB_Insurance,
    SIB_PharmaBiotech,
    SIB_Productivity,
    SIB_Productivity_Info,
    SIB_Mastering_Excel,
    SIB_BecomeProficientInPowerPoint,
    SIB_GettingStartedwithCopilot,
    SIB_OneOnOneMentoring,
    SIB_SelfStudyFreeResources,
    SIB_ViewAllSelfStudy_Btn,
    SIB_PublicCourses_MenuItem,
    SIB_PublicCourses_MenuItem_Info,
    SIB_EssentialComprehensive,
    SIB_AppliedExcel,
    SIB_FinancialStatementAnalysisAndAccounting,
    SIB_PrivateEquityTransition,
    SIB_FinancialModeling,
    SIB_EnhancedEssential,
    SIB_WallStreetBootcamp,
    SIB_PrinciplesOfCapitalMarkets,
    SIB_EveningEssentials,
    SIB_PythonTraining,
    SIB_CopilotFoundations,
    SIB_AdvancedTopics,
    SIB_SoftwareIndustryAnalysisFinancialModeling,
    SIB_RealEstateFundamentals,
    SIB_ViewAllPublicCourses_Btn,
    SIB_CorporateSolutions,
    SIB_CorporateSolutions_Info,
    SIB_CorporateOverview,
    SIB_CorporateExcelTraining,
    SIB_ContinuingDevelopment,
    SIB_ProgramAdvisoryServices,
    SIB_PrivateEquity,
    SIB_GlobalMarkets,
    SIB_FinancialStatementAnalysis,
    SIB_CopilotForFinance,
    SIB_DataScienceAndAnalytics,
    SIB_ModelingSolutions,
    SIB_ProfessionalSkillsTraining,
    SIB_ViewAllCorporateSolutions_Btn,
    SIB_LWRSiteURL,
    SIB_SiteDomainName,
    SIB_StrivacityIdCookieName,
    SIB_ExcelModellingChallenges,
    SIB_PythonCoreDataAnalysis
}